...
const createCollege = async function (req, res) {
  ...
  if (!data.fullName.match(regex1)) return res.status(400).send({ status: false, message: "FULLNAME SHOULD ONLY CONTAIN ALPHABETS AND LENGTH MUST BE IN BETWEEN 2-100"})
  if (!data.logoLink.match(linkregex)) return res.status(400).send({ status: false, message: "LINK SHOULD BE A VALID S3 URL" })
